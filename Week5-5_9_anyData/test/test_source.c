...
START_TEST(test_set)
{
    ...
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After setByte(%d), %s", v1, buffer);
    }
    ...
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After setInt(%d), %s", v2, buffer);
    }
        
    ad = setDouble(5.555);
    ref.type = DOUBLE; ref.value.dval = 5.555;
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After setDouble(%lf), %s", 5.555, buffer);
    }
}
END_TEST
...
START_TEST(test_print)
{
    ...
    if (strcmp(buffer, "B:4")) {
        ...
        fail("After setByte(4) should output '%s', you printed: '%s'",
                "B:4", buffer);
    }
    ...
    if (strcmp(buffer, "I:20000")) {
        remove_nonascii(buffer);
        fail("After setInteger(20000) should output '%s', you printed: '%s'",
                "I:20000", buffer);
    }
    ...
    if (strcmp(buffer, "D:6.666600")) {
        remove_nonascii(buffer);
        fail("After setDouble(6.6666) should output '%s', you printed: '%s'",
                "D:6.666600", buffer);
    }    
}
END_TEST
...
START_TEST(test_sum)
{
    ...
    if (ad.type == UNDEF) {
	    fail("calcSum returned unspecified type");
    }
    AnyData ref;
    char buffer[200];
    ref.type = BYTE; ref.value.bval = 5;
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After countSum{B:2, B:3}, %s", buffer);
    }   

    ad = calcSum(&array[1], &array[3]);
    ref.type = INT; ref.value.ival = 10005;
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After countSum{B:2, B:3, I: 10000}, %s", buffer);
    }   

    ad = calcSum(&array[1], &array[4]);
    ref.type = DOUBLE; ref.value.dval = 10010.555000;
    if (!checkAD(buffer, &ad, &ref)) {
        fail("After countSum{B:2, B:3, I: 10000, D: 10010.555000}, %s", buffer);
    }   
}
END_TEST
...
int main(int argc, const char *argv[])
{
    	...
    	Suite *s = suite_create("anyData");

	tmc_register_test(s, test_set, "anyData_set");
	tmc_register_test(s, test_print, "anyData_print");
	tmc_register_test(s, test_sum, "anyData_sum");
