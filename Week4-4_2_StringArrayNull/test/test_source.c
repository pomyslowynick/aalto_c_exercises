...
{
    char *strarray[] = { "one", "abac", "tammi", "aba", NULL };
    char *searchstring[] = { "two", "one", "tammi", "aba" };
    int  results[] ={-1,0,2,3};
    int location;
    
    for (int i = 0; i < 4; i++) {
        location = locate_string(strarray,searchstring[i]);
...
int main(int argc, const char *argv[])
{
    ...
    Suite *s = suite_create("StringArrayNULL");

    tmc_register_test(s, test_locate_string, "StrArrNullTerm");
